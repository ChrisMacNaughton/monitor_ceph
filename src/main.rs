...
fn log_to_influx(args: &Args, ceph_event: &health::CephHealth) {
    if args.outputs.contains(&"influx".to_string()) && args.influx.is_some() {
        ...
        measurement.add_field("ops", Value::Integer(ceph_event.pgmap.op_per_sec.unwrap_or(0) as i64));
        measurement.add_field("writes", Value::Integer(ceph_event.pgmap.write_bytes_sec.unwrap_or(0) as i64));
        measurement.add_field("reads", Value::Integer(ceph_event.pgmap.read_bytes_sec.unwrap_or(0) as i64));
        ...
        measurement.add_field("osds", Value::Integer(ceph_event.osdmap.osdmap.num_osds as i64));
